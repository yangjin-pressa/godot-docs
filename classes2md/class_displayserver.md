...
Returns the text selection in the [Input Method Editor](https://en.wikipedia.org/wiki/Input_method)_ composition string, with the :ref:`Vector2i<class_Vector2i>`'s ``x`` component being the caret position and ``y`` being the length of the selection.
...
Sets whether [Input Method Editor](https://en.wikipedia.org/wiki/Input_method)_ should be enabled for the window specified by ``window_id``. See also :ref:`window_set_ime_position()<class_DisplayServer_method_window_set_ime_position>`.
...
Sets the position of the [Input Method Editor](https://en.wikipedia.org/wiki/Input_method)_ popup for the specified ``window_id``. Only effective if :ref:`window_set_ime_active()<class_DisplayServer_method_window_set_ime_active>` was set to ``true`` for the specified ``window_id``.
